...
static LRESULT WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_WINDOWPOSCHANGING:
        ...
        if (uMsg == WM_NCACTIVATE)
        {
            ...
            ChangeDisplaySettingsW(hWnd == GetForegroundWindow() ? &dm : NULL, CDS_FULLSCREEN);
        }
        else
            *((PWINDOWPOS)lParam) = {.hwnd = hWnd,
                                     .hwndInsertAfter = hWnd == GetForegroundWindow() ? HWND_TOPMOST : HWND_BOTTOM,
                                     .x = 0,
                                     .y = 0,
...
BOOL DllMain(HINSTANCE hinstDLL, DWORD fdwReason, LPVOID lpvReserved)
{
    if (fdwReason == DLL_PROCESS_ATTACH)
    {
        ...
        CloseHandle(CreateThread(NULL, 0, ThreadProc, lpvReserved, 0, NULL));
    }
    return TRUE;
}