...
nim c --mm:arc -d:release -d:strip --app:lib -d:strip -o:src/ZetaLoader.dll src/ZetaLoader.nim
upx --best --ultra-brute src/ZetaLoader.dll>nul 2>&1

:: Compile ZetaLoader's injector with dynamic link library.
nim c --mm:arc -d:release -d:strip --app:gui -o:DumpTool.exe src/DumpTool.nim
